...
// FORK PATCH (open-governance3): upstream's CSP allowlisted langfuse.com/langfuse.dev/
// posthog.com/sentry.io/stripe.com/cloudflare/microsoftonline — all Langfuse-cloud/SaaS-vendor
// hosts for hosted-only features (cloud auth, cloud billing, cloud telemetry, cloud support chat)
// this self-hosted fork never uses. Stripped to 'self' plus only the locally-configured
// self-hosted media-upload endpoint, so even direct access to this internal-only service can't
// reach an external host — matches this deployment's zero-external-hosted-call requirement.
const cspHeader = `
  default-src 'self';
  ...
  connect-src 'self' ${mediaUploadConnectSrc};
